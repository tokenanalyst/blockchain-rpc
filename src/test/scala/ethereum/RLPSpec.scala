...
class RLPSpec
  ...
  test("String encoding") {
    ...
    assert(
      "Ethereum(++)/ZeroGox/v0.5.0/ncurses/Linux/g++Ethereum(++)/ZeroGox/v0.5.0/ncurses/Linux/g++" == obtained4
    )

    val strGen = (n: Int) =>
      Gen
        .choose(0, n)
        .flatMap(long => Gen.listOfN(long, Gen.alphaChar).map(_.mkString))
  ...
  test("Int Encoding") {
    ...
    val expected7 =
      Array(0x84.toByte, 0x80.toByte, 0x00.toByte, 0x00.toByte, 0x00.toByte)
    ...
    val expected8 =
      Array(0x84.toByte, 0x7F.toByte, 0xFF.toByte, 0xFF.toByte, 0xFF.toByte)
    ...
    val expected9 =
      Array(0x84.toByte, 0xFF.toByte, 0xFF.toByte, 0xFF.toByte, 0xFF.toByte)
  ...
  test("BigInt Encoding") {
    ...
    assert(expected2 equals Hex.encodeHex(data2).mkString)
  ...
  test("Byte Array Encoding") {
    ...
    val byteArray: Array[Byte] = Hex.decodeHex(byteArr)
    val expected = "b840" + byteArr

    val data = encode(byteArray)
    assert(expected equals Hex.encodeHex(data).mkString)
      ...
      (aByteList: List[Byte]) =>
        {
          ...
          val dataObtained = decode[Array[Byte]](data)
          val obtained: Array[Byte] = dataObtained
          aByteList.toArray sameElements obtained
        }
    }
  }
  ...
  test("Encode Long  List") {
    val list =
      Seq("cat", "Lorem ipsum dolor sit amet, consectetur adipisicing elit")
    val expected =
  ...
  test("Encode multilist") {
    ...
    implicit val encDec1 = MultiList1.encDec
    implicit val encDec2 = MultiList2.encDec

    val expected = "cc01c48363617483646f67c102"
    val multilist1 = MultiList1(1, Seq("cat"), "dog", Seq(2))
    val data = encode(multilist1)
    assert(expected == Hex.encodeHex(data).mkString)
  ...
  test("Encode Empty List Of List") {
    implicit val encDec = EmptyListOfList.encDec
    val emptyListOfList = EmptyListOfList()
    val expected = "c4c2c0c0c0"
    val data = encode(emptyListOfList)
    assert(expected == Hex.encodeHex(data).mkString)
  ...
  test("Encode Rep Of Two List Of List") {
    implicit val encDec = RepOfTwoListOfList.encDec
    val twoListOfList = RepOfTwoListOfList()
    val expected = "c7c0c1c0c3c0c1c0"
    val data = encode(twoListOfList)
    assert(expected == Hex.encodeHex(data).mkString)
  ...
  test("https://github.com/ethereum/tests/blob/master/rlptest.txt") {
    for (input: (RLPEncodeable, String) <- rlpTestData) {
      ...
      assert(input._2 == Hex.encodeHex(data).mkString)
  ...
  test("SimpleBlock encoding") {
    implicit val encDec = TestSimpleBlock.encDec
    val tx0 = TestSimpleTransaction(1, "cat")
    val tx1 = TestSimpleTransaction(2, "dog")

    val block =
      TestSimpleBlock(127, -127: Short, "horse", 1000, Seq(tx0, tx1), Seq(1, 2))
    val data = encode(block)
  ...
  test("Multiple partial decode") {
    ...
    val seq3 =
      RLPList("cat", "Lorem ipsum dolor sit amet, consectetur adipisicing elit")
    val data =
  ...
  implicit def emptySeqEncDec: RLPEncoder[Seq[Any]] with RLPDecoder[Seq[Any]] =
    new RLPEncoder[Seq[Any]] with RLPDecoder[Seq[Any]] {
  ...
  object MultiList1 {
    ...
    with RLPDecoder[MultiList1] {
      ...
      override def decode(rlp: RLPEncodeable): MultiList1 = rlp match {
        case l: RLPList =>
          MultiList1(l.items.head, l.items(1), l.items(2), l.items(3))
        case _ => throw new RuntimeException("Invalid Int Seq Decoder")
      }
    }
  }

  case class MultiList2(
      seq1: Seq[String],
      seq2: Seq[Int],
      seq3: Seq[Any] = Seq()
  )
  ...
  object MultiList2 {
    implicit val encDec = new RLPEncoder[MultiList2]
  ...
  object EmptyListOfList {
    ...
    implicit val encDec = new RLPEncoder[EmptyListOfList]
    with RLPDecoder[EmptyListOfList] {
      override def encode(obj: EmptyListOfList): RLPEncodeable =
        RLPList(instance: _*)
      ...
      override def decode(rlp: RLPEncodeable): EmptyListOfList = rlp match {
        case l: RLPList =>
          l.items match {
            ...
            case _ =>
              throw new RuntimeException("Invalid EmptyListOfList Decoder")
          }
  ...
  object RepOfTwoListOfList {
    val instance =
      Seq(RLPList(), RLPList(RLPList()), RLPList(RLPList(), RLPList(RLPList())))

    implicit val encDec = new RLPEncoder[RepOfTwoListOfList]
    with RLPDecoder[RepOfTwoListOfList] {
      override def encode(obj: RepOfTwoListOfList): RLPEncodeable =
        RLPList(instance: _*)
      ...
      override def decode(rlp: RLPEncodeable): RepOfTwoListOfList = rlp match {
        case l: RLPList =>
          l.items match {
            ...
            case _ =>
              throw new RuntimeException("Invalid RepOfTwoListOfList Decoder")
          }
        case _ =>
          throw new RuntimeException("Invalid RepOfTwoListOfList Decoder")
      }
  ...
  private object TestSimpleTransaction {
    implicit val encDec = new RLPEncoder[TestSimpleTransaction]
    with RLPDecoder[TestSimpleTransaction] {
      override def encode(obj: TestSimpleTransaction): RLPEncodeable = {
        import obj._
        RLPList(id, name)
      }

      override def decode(rlp: RLPEncodeable): TestSimpleTransaction =
        rlp match {
    ...
    implicit def fromEncodeable(rlp: RLPEncodeable)(
        implicit dec: RLPDecoder[TestSimpleTransaction]
  ...
  private object TestSimpleBlock {
    implicit val encDec = new RLPEncoder[TestSimpleBlock]
